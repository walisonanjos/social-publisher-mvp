...
import {
  ...
  XCircle,      // Ícone de falha
  Clock,        // Ícone de agendado
} from "lucide-react";
import { useState } from "react";
import Link from "next/link";
import { format, isToday } from "date-fns";
import { ptBR } from "date-fns/locale";
import { Tooltip } from "react-tooltip"; // 1. IMPORTAR O TOOLTIP
...
// Função auxiliar para extrair o erro de uma plataforma específica
...
// Pequeno componente para renderizar o status de cada plataforma
const PlatformStatus = ({ platformName, status, error }: { platformName: 'YouTube' | 'Instagram' | 'Facebook', status: string | null, error: string | null }) => {
  ...
  const statusIcons = {
    agendado: <Clock size={12} className="text-blue-400" />,
    publicado: <CheckCircle size={12} className="text-green-400" />,
    falhou: <XCircle size={12} className="text-red-400" />,
  };
  ...
  return (
    <div className="bg-gray-800/50 p-4 rounded-lg flex flex-col justify-between gap-3 border border-gray-700/80 h-full">
      <div className="flex justify-between items-start">
        ...
        {/* O status geral pode ser removido ou mantido, como preferir. Por ora, vamos manter. */}
      ...
      <div className="flex justify-between items-end mt-auto">
        ...
        <div className="flex items-center gap-3">
          ...
          {isAnyTargetSelected && video.youtube_status === "agendado" && (
...
// O restante do componente VideoGrid (a exportação default) permanece inalterado
