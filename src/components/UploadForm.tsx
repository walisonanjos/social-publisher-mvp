...
export default function UploadForm({
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!file || !title || !scheduleDate) {
      toast.error(t("fill_all_fields")); // <-- TRADUZINDO
      return;
    }
    if (
      !postToYouTube &&
      ...
      toast.error(
        t("select_at_least_one_platform") // <-- TRADUZINDO
      );
      return;
    }
    if (!scheduleTime) {
      toast.error(t("no_available_times")); // <-- TRADUZINDO
      return;
    }
    ...
    try {
      if (!nicheId) throw new Error(t("invalid_workspace_id")); // <-- TRADUZINDO
      ...
      if (isNaN(parsedHours) || isNaN(parsedMinutes)) {
        throw new Error(t("invalid_time_value")); // <-- TRADUZINDO
      }
      ...
      if (existingPost) {
        throw new Error(
          t("appointment_already_exists") // <-- TRADUZINDO
        );
      }
      ...
      if (!cloudinaryResponse.ok)
        throw new Error(t("cloudinary_upload_failed")); // <-- TRADUZINDO
      ...
      if (!user) throw new Error(t("not_authenticated")); // <-- TRADUZINDO
      ...
      if (!newVideo)
        throw new Error(
          t("failed_to_get_appointment") // <-- TRADUZINDO
        );

      toast.success(t("video_scheduled_success")); // <-- TRADUZINDO

      onScheduleSuccess(newVideo as Video, clearFile);
    } catch (err) {
      if (err instanceof Error) {
        toast.error(t("error_occurred", { message: err.message })); // <-- TRADUZINDO COM PLACEHOLDER
      } else {
        toast.error(t("unexpected_error")); // <-- TRADUZINDO
      }
    } finally {
