...
// 1. IMPORTAR O NOVO MODAL E O TOAST (que usaremos para o feedback)
...
export default function NichePageClient({ nicheId }: { nicheId: string }) {
  ...
  // 2. NOVO ESTADO PARA CONTROLAR O MODAL DE EDIÇÃO
  const [editingVideo, setEditingVideo] = useState<Video | null>(null);

  const groupedVideos = useMemo(() => {
    // ... (lógica do groupedVideos inalterada)
 ...
     return groups;
  ...
  const fetchPageData = useCallback(async (userId: string) => {
    // ... (lógica do fetchPageData inalterada)
 ...
     if (nicheData) setNicheName(nicheData.name);
  ...
  useEffect(() => {
    // ... (lógica do useEffect de inicialização inalterada)
 ...
     setupPage();
  ...
  useEffect(() => {
    // ... (lógica do useEffect de Realtime inalterada)
    if (!user) return;
 ...
     };
  ...
  const handleDeleteVideo = async (videoId: string) => {
    // ... (lógica do handleDeleteVideo inalterada)
 ...
     const { error } = await supabase.from('videos').delete().eq('id', videoId);
  ...
  const handleDisconnect = async (platform: 'youtube' | 'instagram') => {
    // ... (lógica do handleDisconnect inalterada, mas podemos usar toasts)
    if (!user) return;
 ...
     }
  ...
  // 3. NOVAS FUNÇÕES PARA CONTROLAR O MODAL DE EDIÇÃO
  ...
  const handleSaveChanges = async (updatedData: { title: string; description: string; scheduled_at: string; }) => {
    ...
    try {
      ...
      setVideos(currentVideos => 
        currentVideos.map(v => v.id === data.id ? data : v)
      );
  ...
  return (
    <div className="bg-gray-900 min-h-screen text-white">
      ...
      <main className="container mx-auto p-4 md:p-8">
        ...
        <div id="upload-form" className="mt-8">
          <UploadForm
            nicheId={nicheId}
        ...
        {/* 5. PASSAMOS A FUNÇÃO PARA O VIDEOGRID */}
        <VideoGrid 
