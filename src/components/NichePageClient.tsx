...
// Removidos: format, isToday, addDays, ptBR — não eram usados
...
export default function NichePageClient({ nicheId }: { nicheId: string }) {
  ...
  useEffect(() => {
    ...
    const handleRealtimeUpdates = (payload: SupabasePayload<Video>) => {
      if (payload.table === "videos") {
        ...
        setVideos((prevVideos) => {
          ...
          if (payload.eventType === "DELETE") {
            return prevVideos.filter((v) => v.id !== oldVideo.id);
          }
          return prevVideos;
        });
      }
  ...
  const handleDisconnect = async (platform: "youtube" | "instagram" | "tiktok") => {
    if (!user) return;
    const platformName =
      platform === "youtube" ? "YouTube" : platform === "instagram" ? "Instagram/Facebook" : "TikTok";
    ...
    if (error) {
      ...
      if (platform === "tiktok") setIsTikTokConnected(false);
    }
  };
